document.addEventListener('DOMContentLoaded', async () => {
  ...
  mediaInput.addEventListener('change', async (e) => {
    ...
    // Validate all files
    for (const currentFile of files) { // <-- Changed variable name here
      const allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'video/mp4', 'video/webm', 'video/quicktime'];
      if (!allowedTypes.includes(currentFile.type)) { // <-- Use currentFile here
        alert(`File ${currentFile.name} is not an allowed type.`);
        return;
      }
      if (currentFile.size > 50 * 1024 * 1024) { // <-- Use currentFile here
    ...
    // Process each file
    for (const currentFile of files) { // <-- Also change here
      const fileExt = currentFile.name.split('.').pop(); // <-- Use currentFile
      ...
      if (uploadProgress && progressFill && progressText) {
        ...
        progressText.textContent = `Uploading ${currentFile.name}...`; // <-- Use currentFile
      }

      // Upload to Supabase storage
      const { error: uploadError } = await supabase.storage
        .from('memories')
        .upload(fileName, currentFile, { // <-- Use currentFile
          upsert: false,
          onUploadProgress: (progressEvent) => {
            ...
            if (progressText) {
              progressText.textContent = `Uploading ${currentFile.name}... ${percentCompleted}%`; // <-- Use currentFile
            }
          }
        });
      ...
      currentMediaFiles.push({
        url: publicUrl,
        name: currentFile.name, // <-- Use currentFile
        type: currentFile.type // <-- Use currentFile
      });

      // Show preview
      showMediaPreview(publicUrl, currentFile.name, currentFile.type); // <-- Use currentFile
    }
